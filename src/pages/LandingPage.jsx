    ...
    <div className="fixed left-4 bottom-4 z-40 hidden md:flex flex-col gap-3">
      {socialLinks.map((social, index) => (
        <motion.a
          ...
          className={`p-3 bg-white dark:bg-gray-800 rounded-full shadow-lg text-gray-600 dark:text-gray-400 ${social.color} transition-all duration-300 touch-manipulation`}
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <motion.div
            className="bg-white dark:bg-gray-800 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden mx-2"
          ...
          >
            <div className="flex justify-between items-center p-4 border-b dark:border-gray-700">
              <h3 className="text-lg md:text-xl font-bold text-gray-800 dark:text-white pr-4">
                {certificate.title}
              </h3>
              <button
                onClick={onClose}
                ...
                <div className="mt-6 text-center w-full">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    <a
                      ...
                      download
                      className="flex items-center justify-center gap-2 px-4 py-3 bg-pink-500 hover:bg-pink-600 text-white rounded-lg transition-colors text-sm md:text-base"
                    >
                    ...
                    <button
                      onClick={onClose}
                      className="px-4 py-3 bg-gray-500 hover:bg-gray-600 text-white rounded-lg transition-colors text-sm md:text-base"
                    >
                      Tutup
                    </button>
                  </div>
...
function LandingPage() {
  ...
  useEffect(() => {
    ...
    // Menambahkan viewport meta tag secara dinamis untuk mobile
  ...
  const handleNavClick = (href) => {
    ...
    if (href.startsWith('#')) {
      ...
      if (targetElement) {
        setTimeout(() => {
          const offsetTop = targetElement.offsetTop - 80; // Reduced for mobile
          window.scrollTo({
            top: offsetTop,
            behavior: 'smooth'
          });
...
const educationData = [
  ...
  const certificationsData = [
    ...
    },
    {
      ...
      date: "mei 2024",
      credentialLink: "#",
      image: "/sertifikat fundamental frontend.jpg"
    },
    {
    ...
    <div className={`min-h-screen ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gradient-to-br from-pink-50 via-pink-50 to-pink-100 text-gray-800'} font-['Poppins'] selection:bg-pink-500 selection:text-white overflow-x-hidden transition-colors duration-300`}>
      ...
      <header className={`fixed top-0 w-full ${darkMode ? 'bg-gray-900' : 'bg-pink-50'} bg-opacity-90 backdrop-blur-md z-50 shadow-lg border-b ${darkMode ? 'border-gray-800' : 'border-pink-200'} transition-colors duration-300`}>
        <nav className="max-w-6xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex justify-between items-center">
          ...
          >
            ...
            <h1 className="text-lg sm:text-xl font-bold tracking-widest uppercase">PORTFOLIO IMELDA</h1>
          </motion.div>
          ...
          {/* Mobile Nav Toggle */}
          <motion.button
            className={`md:hidden ${darkMode ? 'text-gray-400 hover:text-pink-400' : 'text-pink-700 hover:text-pink-500'} transition text-2xl p-1`}
        ...
        <AnimatePresence>
          {navOpen && (
            <motion.ul
              ...
              className={`md:hidden ${darkMode ? 'bg-gray-800' : 'bg-pink-50'} px-4 sm:px-6 py-4 space-y-3 text-lg font-medium transition-colors duration-300 overflow-hidden`}
            >
              {navLinks.map(({ href, label, icon }) => (
                <motion.li 
                  key={label}
              ...
              >
                <button
                  ...
                  aria-label="Toggle dark mode"
                  className={`${darkMode ? 'text-gray-400 hover:text-pink-400' : 'text-pink-700 hover:text-pink-500'} transition text-xl flex items-center gap-2 py-2`}
                >
      ...
      <main className="pt-20 sm:pt-24 md:pt-32">
        ...
        <section className="min-h-screen flex flex-col justify-center relative overflow-hidden">
          ...
          <motion.div 
            className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-20 text-center"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            >
              ...
              <motion.span
                ...
                className="bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-purple-600 text-lg sm:text-xl md:text-2xl font-medium mt-2 sm:mt-4 block"
              >
            ...
            >
              <motion.a
                href="#projects"
                className="px-6 sm:px-8 py-3 sm:py-4 bg-pink-600 hover:bg-pink-700 text-white rounded-lg font-medium transition-colors shadow-lg flex items-center justify-center gap-2 text-sm sm:text-base"
              ...
              <motion.a
                href="#contact"
                className={`px-6 sm:px-8 py-3 sm:py-4 ${darkMode ? 'bg-gray-800 hover:bg-gray-700 text-white' : 'bg-white hover:bg-pink-100 text-pink-800'} rounded-lg font-medium transition-colors border ${darkMode ? 'border-gray-700' : 'border-pink-300'} shadow-lg flex items-center justify-center gap-2 text-sm sm:text-base`}
                whileHover={{ y: -3, scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                onClick={(e) => {
                  e.preventDefault();
            ...
            >
              ...
              >
                <FaChevronDown size={20} className="sm:w-6 sm:h-6" />
              </a>
            </motion.div>
          </motion.div>
        </section>
        ...
        {/* About Section */}
        <section id="about" className={`py-16 sm:py-20 ${darkMode ? 'bg-gray-900' : 'bg-pink-50'} transition-colors duration-300`}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            ...
            >
              <h2 className={`text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 ${darkMode ? 'text-white' : 'text-pink-800'} font-['Montserrat']`}>
                <FaUser className="inline mr-2 sm:mr-3 text-pink-500" /> Tentang Saya
              </h2>
              <div className="w-16 sm:w-20 h-1 bg-pink-500 mx-auto mb-4 sm:mb-6"></div>
              <p className={`${darkMode ? 'text-gray-400' : 'text-pink-700'} max-w-2xl mx-auto text-base sm:text-lg px-4`}>
            ...
            <div className="grid md:grid-cols-2 gap-8 sm:gap-12 items-center">
              ...
              >
                <div className="relative max-w-xs">
                  <motion.div 
                    className="absolute -inset-2 sm:-inset-3 border-2 border-pink-500 rounded-xl transform rotate-3"
                  ...
                  <img
                    src="/mee.png"
                    alt="Imelda Novianty"
                    className="relative z-10 rounded-xl shadow-2xl transform -rotate-3 w-64 h-64 sm:w-72 sm:h-72 object-cover"
                    onError={(e) => {
              ...
              >
                <h3 className="text-xl sm:text-2xl font-bold text-pink-500 font-['Montserrat']">Imelda Novianty</h3>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  <motion.div 
                    className="flex flex-col p-3 sm:p-4 rounded-lg bg-white dark:bg-gray-800 shadow-md"
                    whileHover={{ y: -3, scale: 1.01 }}
                    transition={{ type: "spring", stiffness: 300 }}
                  >
                    <div className="flex items-center gap-2 mb-1 sm:mb-2">
                      <FaGraduationCap className="text-pink-500 text-sm sm:text-base" />
                      <span className="font-semibold text-sm sm:text-base">Pendidikan:</span>
                    </div>
                    <p className={`text-xs sm:text-sm ${darkMode ? "text-gray-300" : "text-pink-700"}`}>Management Informatika - Universitas Nasional Pasim</p>
                  </motion.div>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1 sm:mb-2">
                      <FaLaptopCode className="text-pink-500 text-sm sm:text-base" />
                      <span className="font-semibold text-sm sm:text-base">Peran:</span>
                    </div>
                    <p className={`text-xs sm:text-sm ${darkMode ? "text-gray-300" : "text-pink-700"}`}>Junior Programmer</p>
                  </motion.div>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1 sm:mb-2">
                      <FaMapMarkerAlt className="text-pink-500 text-sm sm:text-base" />
                      <span className="font-semibold text-sm sm:text-base">Lokasi:</span>
                    </div>
                    <p className={`text-xs sm:text-sm ${darkMode ? "text-gray-300" : "text-pink-700"}`}>Bandung, Jawa Barat</p>
                  </motion.div>
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1 sm:mb-2">
                      ...
                      <span className="font-semibold text-sm sm:text-base">Minat:</span>
                    </div>
                    <p className={`text-xs sm:text-sm ${darkMode ? "text-gray-300" : "text-pink-700"}`}>
                ...
                <motion.p 
                  className={`${darkMode ? "text-gray-300" : "text-pink-700"} text-sm sm:text-base`}
                  initial={{ opacity: 0 }}
                ...
                >
                  <a
                    href="#contact"
                    className="px-4 sm:px-6 py-2 sm:py-3 bg-pink-600 hover:bg-pink-700 text-white rounded-lg font-medium transition-colors shadow-md text-center text-sm sm:text-base"
                    onClick={(e) => {
                      e.preventDefault();
                      handleNavClick("#contact");
                    }}
                  ...
                  <a
                    href="#projects"
                    className={`px-4 sm:px-6 py-2 sm:py-3 ${darkMode ? 'bg-gray-800 hover:bg-gray-700 text-white' : 'bg-white hover:bg-pink-100 text-pink-800'} rounded-lg font-medium transition-colors border ${darkMode ? 'border-gray-700' : 'border-pink-300'} shadow-md text-center text-sm sm:text-base`}
                    onClick={(e) => {
                      e.preventDefault();
                      handleNavClick("#projects");
                    }}
        ...
        {/* Education Section */}
        <section id="education" className={`py-16 sm:py-20 ${darkMode ? 'bg-gray-800' : 'bg-white'} transition-colors duration-300`}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <h2 className={`text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 ${darkMode ? 'text-white' : 'text-pink-800'} font-['Montserrat']`}>
                <FaGraduationCap className="inline mr-2 sm:mr-3 text-pink-500" /> Pendidikan & Sertifikasi
              </h2>
              <div className="w-16 sm:w-20 h-1 bg-pink-500 mx-auto mb-4 sm:mb-6"></div>
              <p className={`${darkMode ? 'text-gray-400' : 'text-pink-700'} max-w-2xl mx-auto text-base sm:text-lg px-4`}>
            ...
            <div className="grid md:grid-cols-2 gap-8 sm:gap-12">
              ...
              <motion.div
                ...
                transition={{ duration: 0.8 }}
              >
                <h3 className="text-xl sm:text-2xl font-bold text-pink-500 mb-6 sm:mb-8 font-['Montserrat']">Pendidikan Formal</h3>
                
                <div className="space-y-6 sm:space-y-8">
                  {educationData.map((edu, index) => (
                    <motion.div 
                      key={index}
                      className={`relative pl-8 sm:pl-10 pb-6 sm:pb-8 ${index !== educationData.length - 1 ? 'border-l-2 border-pink-500' : ''}`}
                      initial={{ opacity: 0, y: 20 }}
                      whileInView={{ opacity: 1, y: 0 }}
                      viewport={{ once: true, margin: "-50px" }}
                      transition={{ duration: 0.8, delay: index * 0.1 }}
                    >
                      <div className="absolute -left-1.5 sm:-left-2 top-0 w-3 h-3 sm:w-4 sm:h-4 rounded-full bg-pink-500"></div>
                      
                      <div className={`p-4 sm:p-6 rounded-xl ${darkMode ? 'bg-gray-700' : 'bg-pink-50'} shadow-md`}>
                        <h4 className="text-lg sm:text-xl font-bold mb-1 sm:mb-2">{edu.degree}</h4>
                        <p className={`font-semibold mb-1 sm:mb-2 ${darkMode ? 'text-pink-400' : 'text-pink-600'} text-sm sm:text-base`}>{edu.institution}</p>
                        <p className={`mb-2 sm:mb-3 ${darkMode ? 'text-gray-400' : 'text-pink-700'} text-xs sm:text-sm`}>{edu.period}</p>
                        <p className={`mb-3 sm:mb-4 ${darkMode ? 'text-gray-300' : 'text-pink-800'} text-sm sm:text-base`}>{edu.description}</p>
                        
                        <div className="mt-3 sm:mt-4">
                          <h5 className="font-semibold mb-1 sm:mb-2 flex items-center gap-2 text-sm sm:text-base">
                            <FaAward className="text-pink-500" /> Prestasi & Pencapaian
                          </h5>
                          <ul className={`space-y-1 sm:space-y-2 ${darkMode ? 'text-gray-300' : 'text-pink-700'} text-xs sm:text-sm`}>
              ...
              <motion.div
                ...
                transition={{ duration: 0.8, delay: 0.2 }}
              >
                <h3 className="text-xl sm:text-2xl font-bold text-pink-500 mb-6 sm:mb-8 font-['Montserrat']">Sertifikasi</h3>
                
                <div className="space-y-4 sm:space-y-6 max-h-[500px] overflow-y-auto pr-2">
                  {certificationsData.map((cert, index) => (
                    <motion.div 
                      key={index}
                      className={`p-4 sm:p-6 rounded-xl ${darkMode ? 'bg-gray-700' : 'bg-pink-50'} shadow-md flex items-start gap-3 sm:gap-4 cursor-pointer`}
                      initial={{ opacity: 0, y: 20 }}
                      whileInView={{ opacity: 1, y: 0 }}
                      viewport={{ once: true, margin: "-50px" }}
                      transition={{ duration: 0.8, delay: index * 0.1 }}
                    ...
                    >
                      <div className="p-2 sm:p-3 bg-pink-100 dark:bg-pink-900 rounded-full flex-shrink-0">
                        <FaCertificate className="text-pink-500 text-lg sm:text-xl" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <h4 className="text-base sm:text-lg font-bold mb-1 truncate">{cert.title}</h4>
                        <p className={`mb-1 ${darkMode ? 'text-gray-400' : 'text-pink-700'} text-xs sm:text-sm truncate`}>{cert.issuer}</p>
                        <p className={`mb-2 ${darkMode ? 'text-gray-500' : 'text-pink-600'} text-xs`}>{cert.date}</p>
                        <button 
                          className="text-pink-500 hover:text-pink-600 text-xs sm:text-sm font-medium flex items-center gap-1"
                        >
                ...
                <motion.div 
                  className={`mt-6 sm:mt-8 p-4 sm:p-6 rounded-xl ${darkMode ? 'bg-gray-700' : 'bg-pink-100'} shadow-md`}
                  initial={{ opacity: 0 }}
                  whileInView={{ opacity: 1 }}
                  viewport={{ once: true, margin: "-50px" }}
                  transition={{ duration: 0.8, delay: 0.4 }}
                >
                  <h4 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 text-pink-500">Keterampilan Tambahan</h4>
                  <div className="grid grid-cols-2 gap-3 sm:gap-4">
                    <div className={`p-2 sm:p-3 rounded-lg ${darkMode ? 'bg-gray-600' : 'bg-white'} text-center shadow-sm`}>
                      <p className="font-semibold text-sm sm:text-base">Bahasa Inggris</p>
                      <p className={`text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-pink-700'}`}>Menengah</p>
                    </div>
                    <div className={`p-2 sm:p-3 rounded-lg ${darkMode ? 'bg-gray-600' : 'bg-white'} text-center shadow-sm`}>
                      <p className="font-semibold text-sm sm:text-base">Manajemen Proyek</p>
                      <p className={`text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-pink-700'}`}>Baik</p>
                    </div>
                    <div className={`p-2 sm:p-3 rounded-lg ${darkMode ? 'bg-gray-600' : 'bg-white'} text-center shadow-sm`}>
                      <p className="font-semibold text-sm sm:text-base">Komunikasi</p>
                      <p className={`text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-pink-700'}`}>Ahli</p>
                    </div>
                    <div className={`p-2 sm:p-3 rounded-lg ${darkMode ? 'bg-gray-600' : 'bg-white'} text-center shadow-sm`}>
                      <p className="font-semibold text-sm sm:text-base">Kerja Tim</p>
                      <p className={`text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-pink-700'}`}>Ahli</p>
                    </div>
                  </div>
                </motion.div>
        ...
        {/* Skills Section */}
        <section id="skills" className={`py-16 sm:py-20 ${darkMode ? 'bg-gray-900' : 'bg-pink-50'} transition-colors duration-300`}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <h2 className={`text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 ${darkMode ? 'text-white' : 'text-pink-800'} font-['Montserrat']`}>
                <FaCode className="inline mr-2 sm:mr-3 text-pink-500" /> Keahlian Teknis
              </h2>
              <div className="w-16 sm:w-20 h-1 bg-pink-500 mx-auto mb-4 sm:mb-6"></div>
              <p className={`${darkMode ? 'text-gray-400' : 'text-pink-700'} max-w-2xl mx-auto text-base sm:text-lg px-4`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6">
              {skillsData.map((skill, index) => {
                ...
                return (
                  <motion.div
                    key={index}
                    className={`p-4 sm:p-6 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-md text-center group cursor-default`}
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true, margin: "-50px" }}
                  ...
                  >
                    <div className="flex justify-center mb-3 sm:mb-4">
                      <IconComponent className={`text-2xl sm:text-3xl md:text-4xl ${darkMode ? 'text-pink-400' : 'text-pink-600'} group-hover:text-pink-500 transition-colors`} />
                    </div>
                    <h3 className="font-semibold text-sm sm:text-base mb-2">{skill.name}</h3>
            ...
            <motion.div 
              className={`mt-12 sm:mt-16 p-6 sm:p-8 rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-md`}
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: "-50px" }}
              transition={{ duration: 0.8, delay: 0.2 }}
            >
              <h3 className="text-xl sm:text-2xl font-bold text-pink-500 mb-4 sm:mb-6 text-center font-['Montserrat']">Pengalaman Pengembangan</h3>
              
              <div className="grid md:grid-cols-3 gap-4 sm:gap-6">
                <div className={`p-4 sm:p-6 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-pink-50'} text-center`}>
                  <div className="text-2xl sm:text-4xl text-pink-500 mb-3 sm:mb-4">🚀</div>
                  <h4 className="font-semibold mb-1 sm:mb-2 text-sm sm:text-base">Pengembangan Frontend</h4>
                  <p className={`text-xs sm:text-sm ${darkMode ? "text-gray-300" : "text-pink-700"}`}>
                ...
                <div className={`p-4 sm:p-6 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-pink-50'} text-center`}>
                  <div className="text-2xl sm:text-4xl text-pink-500 mb-3 sm:mb-4">⚙️</div>
                  <h4 className="font-semibold mb-1 sm:mb-2 text-sm sm:text-base">Pengembangan Backend</h4>
                  <p className={`text-xs sm:text-sm ${darkMode ? "text-gray-300" : "text-pink-700"}`}>
                ...
                <div className={`p-4 sm:p-6 rounded-lg ${darkMode ? 'bg-gray-700' : 'bg-pink-50'} text-center`}>
                  <div className="text-2xl sm:text-4xl text-pink-500 mb-3 sm:mb-4">🎨</div>
                  <h4 className="font-semibold mb-1 sm:mb-2 text-sm sm:text-base">UI/UX Design</h4>
                  <p className={`text-xs sm:text-sm ${darkMode ? "text-gray-300" : "text-pink-700"}`}>
        ...
        {/* Projects Section */}
        <section id="projects" className={`py-16 sm:py-20 ${darkMode ? 'bg-gray-800' : 'bg-white'} transition-colors duration-300`}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <h2 className={`text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 ${darkMode ? 'text-white' : 'text-pink-800'} font-['Montserrat']`}>
                <FaLaptopCode className="inline mr-2 sm:mr-3 text-pink-500" /> Proyek Terbaru
              </h2>
              <div className="w-16 sm:w-20 h-1 bg-pink-500 mx-auto mb-4 sm:mb-6"></div>
              <p className={`${darkMode ? 'text-gray-400' : 'text-pink-700'} max-w-2xl mx-auto text-base sm:text-lg px-4`}>
              ...
              {projectsData.map((project, index) => (
                ...
                >
                  <div className="relative overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 bg-black bg-opacity-60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 sm:gap-4">
                      <a 
                        href={project.githubUrl} 
                        target="_blank"
                        rel="noopener noreferrer"
                        className="p-2 sm:p-3 bg-white rounded-full text-gray-800 hover:bg-pink-500 hover:text-white transition-colors"
                  ...
                  <div className="p-4 sm:p-6">
                    <div className="flex items-center justify-between mb-2 sm:mb-3">
                      <span className={`text-xs font-semibold px-2 py-1 rounded-full ${darkMode ? 'bg-pink-900 text-pink-200' : 'bg-pink-200 text-pink-800'}`}>
                        {project.category}
                      </span>
                    </div>
                    
                    <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3">{project.title}</h3>
                    <p className={`mb-3 sm:mb-4 ${darkMode ? 'text-gray-300' : 'text-pink-700'} text-sm sm:text-base line-clamp-3`}>{project.description}</p>
                    
                    <div className="mb-4 sm:mb-5 flex flex-wrap gap-1 sm:gap-2">
                    ...
                    <div className="flex justify-between items-center">
                      <a 
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`flex items-center gap-1 sm:gap-2 ${darkMode ? 'text-pink-400 hover:text-pink-300' : 'text-pink-600 hover:text-pink-700'} font-medium text-sm sm:text-base`}
                      >
                        <FaGithub className="text-xs sm:text-sm" /> Kode Sumber
                      </a>
                      ...
                      {project.liveUrl !== "#" && (
                        <a 
                          ...
                          className={`flex items-center gap-1 sm:gap-2 ${darkMode ? 'text-pink-400 hover:text-pink-300' : 'text-pink-600 hover:text-pink-700'} font-medium text-sm sm:text-base`}
                        >
                          Demo <FaExternalLinkAlt size={10} className="sm:w-3 sm:h-3" />
                        </a>
                      )}
                    </div>
                  </div>
            ...
            >
              <a 
                ...
                target="_blank"
                rel="noopener noreferrer"
                className={`inline-flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-3 rounded-lg ${darkMode ? 'bg-gray-700 hover:bg-gray-600 text-white' : 'bg-pink-100 hover:bg-pink-200 text-pink-800'} font-medium transition-colors text-sm sm:text-base`}
              >
        ...
        {/* Contact Section */}
        <section id="contact" className={`py-16 sm:py-20 ${darkMode ? 'bg-gray-900' : 'bg-pink-50'} transition-colors duration-300`}>
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <motion.div
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <h2 className={`text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 ${darkMode ? 'text-white' : 'text-pink-800'} font-['Montserrat']`}>
                <FaPaperPlane className="inline mr-2 sm:mr-3 text-pink-500" /> Hubungi Saya
              </h2>
              <div className="w-16 sm:w-20 h-1 bg-pink-500 mx-auto mb-4 sm:mb-6"></div>
              <p className={`${darkMode ? 'text-gray-400' : 'text-pink-700'} max-w-2xl mx-auto text-base sm:text-lg px-4`}>
            ...
            <div className="grid md:grid-cols-2 gap-8 sm:gap-12">
              <motion.div
                ...
                transition={{ duration: 0.8 }}
              >
                <h3 className="text-xl sm:text-2xl font-bold text-pink-500 mb-4 sm:mb-6 font-['Montserrat']">Informasi Kontak</h3>
                
                <div className="space-y-4 sm:space-y-6">
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="p-2 sm:p-3 bg-pink-100 dark:bg-pink-900 rounded-full flex-shrink-0">
                      <FaEnvelope className="text-pink-500 text-lg sm:text-xl" />
                    </div>
                    <div>
                      <h4 className="font-semibold text-sm sm:text-base">Email</h4>
                      <a 
                        href="mailto:noviantyimelda@gmail.com" 
                        className={`${darkMode ? 'text-gray-300 hover:text-pink-400' : 'text-pink-700 hover:text-pink-500'} transition-colors text-sm sm:text-base break-all`}
                      >
                  ...
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="p-2 sm:p-3 bg-pink-100 dark:bg-pink-900 rounded-full flex-shrink-0">
                      <FaWhatsapp className="text-pink-500 text-lg sm:text-xl" />
                    </div>
                    <div>
                      ...
                      <a 
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`${darkMode ? 'text-gray-300 hover:text-pink-400' : 'text-pink-700 hover:text-pink-500'} transition-colors text-sm sm:text-base`}
                      >
                        +62 852 2328 4793
                      </a>
                    </div>
                  ...
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="p-2 sm:p-3 bg-pink-100 dark:bg-pink-900 rounded-full flex-shrink-0">
                      <FaMapMarkerAlt className="text-pink-500 text-lg sm:text-xl" />
                    </div>
                    <div>
                      <h4 className="font-semibold text-sm sm:text-base">Lokasi</h4>
                      <p className={`text-sm sm:text-base ${darkMode ? "text-gray-300" : "text-pink-700"}`}>Bandung, Jawa Barat, Indonesia</p>
                    </div>
                  </div>
                  
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="p-2 sm:p-3 bg-pink-100 dark:bg-pink-900 rounded-full flex-shrink-0">
                      <FaLinkedin className="text-pink-500 text-lg sm:text-xl" />
                    </div>
                    <div>
                ...
                <div className="mt-6 sm:mt-8">
                  <h4 className="text-lg sm:text-xl font-bold text-pink-500 mb-3 sm:mb-4 font-['Montserrat']">Media Sosial</h4>
                  <div className="flex gap-3 sm:gap-4">
                    {[
                      ...
                      <motion.a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`p-2 sm:p-3 ${darkMode ? 'bg-gray-800 text-gray-300 hover:bg-pink-600' : 'bg-pink-100 text-pink-700 hover:bg-pink-200'} rounded-full transition-colors`}
              ...
              <motion.div
                ...
                transition={{ duration: 0.8, delay: 0.2 }}
              >
                <h3 className="text-xl sm:text-2xl font-bold text-pink-500 mb-4 sm:mb-6 font-['Montserrat']">Kirim Pesan</h3>
                
                <form className="space-y-4 sm:space-y-6" onSubmit={handleFormSubmit}>
                  <div className="grid md:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                      <label htmlFor="name" className={`block mb-1 sm:mb-2 font-medium ${darkMode ? 'text-gray-300' : 'text-pink-700'} text-sm sm:text-base`}>Nama Lengkap</label>
                      <input 
                        type="text" 
                        id="name" 
                        value={formData.name}
                        onChange={handleFormChange}
                        className={`w-full px-3 sm:px-4 py-2 sm:py-3 rounded-lg ${darkMode ? 'bg-gray-800 text-white border-gray-700' : 'bg-white text-pink-800 border-pink-200'} border focus:ring-2 focus:ring-pink-500 focus:border-transparent outline-none transition-colors text-sm sm:text-base`}
                        placeholder="Nama Anda"
                        required
                      />
                    </div>
                    <div>
                      <label htmlFor="email" className={`block mb-1 sm:mb-2 font-medium ${darkMode ? 'text-gray-300' : 'text-pink-700'} text-sm sm:text-base`}>Alamat Email</label>
                      <input 
                        type="email" 
                        id="email" 
                  ...
                  <div>
                    <label htmlFor="subject" className={`block mb-1 sm:mb-2 font-medium ${darkMode ? 'text-gray-300' : 'text-pink-700'} text-sm sm:text-base`}>Subjek</label>
                    <input 
                      type="text" 
                      id="subject" 
                  ...
                  <div>
                    <label htmlFor="message" className={`block mb-1 sm:mb-2 font-medium ${darkMode ? 'text-gray-300' : 'text-pink-700'} text-sm sm:text-base`}>Pesan</label>
                    <textarea 
                      id="message" 
                      rows="4"
                  ...
                  <motion.button
                    type="submit"
                    className="w-full py-2 sm:py-3 bg-pink-600 hover:bg-pink-700 text-white rounded-lg font-medium transition-colors shadow-md flex items-center justify-center gap-1 sm:gap-2 text-sm sm:text-base"
                    whileHover={{ y: -2 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <FaPaperPlane className="text-xs sm:text-sm" /> Kirim Pesan
                  </motion.button>
                </form>
              </motion.div>
            </div>
      ...
      {/* Footer */}
      <footer className={`py-8 sm:py-12 ${darkMode ? 'bg-gray-800' : 'bg-pink-100'} transition-colors duration-300`}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center">
          ...
          >
            <h3 className="text-xl sm:text-2xl font-bold text-pink-500 mb-3 sm:mb-4 font-['Montserrat']">Imelda Novianty</h3>
            <p className={`max-w-2xl mx-auto ${darkMode ? 'text-gray-400' : 'text-pink-700'} mb-4 sm:mb-6 text-sm sm:text-base px-4`}>
            ...
            <div className="flex justify-center gap-4 sm:gap-6 mb-6 sm:mb-8">
              {[
                ...
                <motion.a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`p-2 sm:p-3 ${darkMode ? 'bg-gray-700 text-gray-300 hover:bg-pink-600' : 'bg-pink-200 text-pink-700 hover:bg-pink-300'} rounded-full transition-colors`}
                  whileHover={{ y: -3, scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  aria-label={social.label}
                >
          ...
          <div className={`pt-6 sm:pt-8 border-t ${darkMode ? 'border-gray-700' : 'border-pink-200'}`}>
            <p className={`${darkMode ? 'text-gray-400' : 'text-pink-700'} text-sm sm:text-base`}>
